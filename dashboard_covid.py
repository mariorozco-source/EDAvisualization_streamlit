...
<style>
    ...
    /* Forzar modo claro en todos los componentes custom — evita que el modo oscuro
       del dispositivo invierta o elimine colores en tarjetas y textos personalizados */
    html, body, [class*="css"] {
    ...
    .banner h1 { color: #ffffff !important; margin: 0 0 0.4rem 0; font-size: 1.75rem; font-weight: 700; }
    .banner p  { color: rgba(255,255,255,0.92) !important; margin: 0; font-size: 0.95rem; line-height: 1.55; }
    .banner-blue   { background: linear-gradient(135deg, #1a6bb5 0%, #0e4f8b 100%) !important; box-shadow: 0 4px 20px rgba(26,107,181,0.35); }
    .banner-green  { background: linear-gradient(135deg, #1e8449 0%, #145a32 100%) !important; box-shadow: 0 4px 20px rgba(30,132,73,0.35); }
    .banner-purple { background: linear-gradient(135deg, #6c3483 0%, #4a235a 100%) !important; box-shadow: 0 4px 20px rgba(108,52,131,0.35); }
    .banner-dark   { background: linear-gradient(135deg, #2c3e50 0%, #1a252f 100%) !important; box-shadow: 0 4px 20px rgba(44,62,80,0.35); }
    ...
    .metric-value { font-size: 2.1rem; font-weight: 700; color: #1a1a2e !important; margin: 0.3rem 0 0.2rem; }
    .metric-label { font-size: 0.82rem; color: #5a6371 !important; font-weight: 500; text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .card h3, .card h4 { color: #1a1a2e !important; }
    .card p, .card li, .card td, .card th { color: #374151 !important; }
    ...
    .interpretacion b { color: #c0392b !important; }
    .interpretacion.blue   { border-left-color: #2980b9; background: #f0f7ff !important; }
    .interpretacion.blue b { color: #2980b9 !important; }
    .interpretacion.green  { border-left-color: #27ae60; background: #f0fff6 !important; }
    .interpretacion.green b { color: #1e8449 !important; }
    .interpretacion.purple { border-left-color: #8e44ad; background: #faf0ff !important; }
    .interpretacion.purple b { color: #8e44ad !important; }
    ...
    .question-card h4 { margin: 0 0 0.5rem; font-size: 0.98rem; font-weight: 600; color: #1a1a2e !important; }
    .question-card p  { margin: 0; font-size: 0.87rem; color: #4b5563 !important; line-height: 1.5; }
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if df_h1.empty:
        ...
        interpretacion(
            "<b>Cómo leer este gráfico:</b> Cada figura muestra la distribución de edades. "
            "Cuánto más amplia sea la forma, más personas tienen esa edad. La línea del medio indica la edad típica del grupo.<br><br>"
            "<b>Lo que encontramos:</b> En ambos géneros, las personas que fallecieron tenían en promedio más edad que quienes sobrevivieron. "
            "En hombres, los fallecidos se concentran entre los 60 y 80 años. Las mujeres muestran un patrón similar, "
            "con una edad promedio de fallecimiento ligeramente mayor, lo que podría sugerir una mayor longevidad.",
            "blue"
        )
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if df_h2.empty:
        ...
        interpretacion(
            "<b>Cómo leer este gráfico:</b> Cada caja muestra la distribución de días que tardaron los pacientes en ir al hospital. "
            "La línea del centro es el valor típico (mediana) y los puntos son casos individuales.<br><br>"
            "<b>Lo que encontramos:</b> Los pacientes que fallecieron tardaron en promedio más días en llegar al hospital "
            "desde que empezaron con síntomas. Esto sugiere que una atención médica más oportuna pudo estar relacionada "
            "con mayores posibilidades de sobrevivir. Sin embargo, la edad y las condiciones previas de salud "
            "también son factores determinantes en el desenlace.",
            "green"
        )
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    interpretacion(
        "<b>Lo que encontramos:</b> China fue el país con más casos confirmados y fallecidos en este período, "
        "lo cual es consistente con ser el origen del brote. Singapur presentó un número considerable de casos "
        "pero con más recuperados que fallecidos. Alemania, Tailandia y España no registraron muertes en este corte de tiempo.<br><br>"
        "Es importante recordar que muchos de estos casos aún estaban activos cuando se recopilaron los datos, "
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    if df_plot.empty:
        ...
        interpretacion(
            ...
            "que habían visitado Wuhan, lo que confirma que el virus salió de ese epicentro a través de viajeros. "
            "China concentró el mayor número de casos desde el inicio. "
            "Japón registró un número significativo de casos, muchos sin vínculo directo con Wuhan, "
            "lo que podría indicar transmisión comunitaria temprana.<br><br>"
            "Activa la opción <em>Distinguir vínculo con Wuhan</em> para ver cómo cambia el patrón "
            "según la relación de cada paciente con la ciudad epicentro del brote.",
            "purple"
        )
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with col_a:
        st.markdown("""
        <div class="card" style="color-scheme:light;">
        <p style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#64748b !important;margin-bottom:0.8rem;">Fuente principal</p>
        <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
            <tr><td style="padding:5px 0;color:#64748b !important;width:45%">Nombre</td><td style="color:#1e293b !important;font-weight:500">COVID-19 Line List Data</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Plataforma</td><td style="color:#1e293b !important;font-weight:500">Kaggle</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Autor</td><td style="color:#1e293b !important;font-weight:500">pratik1235</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Archivo</td><td style="color:#1e293b !important;font-weight:500">COVID19_line_list_data.csv</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Fecha de acceso</td><td style="color:#1e293b !important;font-weight:500">26 de febrero de 2026</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Período cubierto</td><td style="color:#1e293b !important;font-weight:500">Ene 22, 2020 — Feb 2020</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Tipo de datos</td><td style="color:#1e293b !important;font-weight:500">Un registro por paciente</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Referencia</td><td style="color:#1e293b !important;font-weight:500">CDC</td></tr>
        </table>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="card" style="color-scheme:light;">
        <p style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#64748b !important;margin-bottom:0.8rem;">Herramientas utilizadas</p>
        <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
            <tr><td style="padding:5px 0;color:#64748b !important;width:45%">Python 3</td><td style="color:#1e293b !important;">Lenguaje de programación principal</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Pandas</td><td style="color:#1e293b !important;">Procesamiento y limpieza de datos</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Plotly</td><td style="color:#1e293b !important;">Visualizaciones interactivas</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">Streamlit</td><td style="color:#1e293b !important;">Construcción y despliegue del tablero</td></tr>
            <tr><td style="padding:5px 0;color:#64748b !important;">NumPy</td><td style="color:#1e293b !important;">Cálculos numéricos auxiliares</td></tr>
        </table>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_b:
        st.markdown("""
        <div class="card" style="color-scheme:light;">
        <p style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#64748b !important;margin-bottom:0.8rem;">¿Qué contiene la base de datos?</p>
        <p style="font-size:0.9rem;color:#374151 !important;line-height:1.65;">
        El dataset registra información a nivel individual para cada caso confirmado durante
        las primeras semanas del brote. Cada fila representa un paciente e incluye:
        </p>
        <ul style="font-size:0.9rem;color:#374151 !important;line-height:1.9;padding-left:1.2rem;">
            <li><strong>Datos personales:</strong> edad, género y país.</li>
            <li><strong>Exposición al virus:</strong> si el paciente vivía en Wuhan o la había visitado.</li>
            <li><strong>Cronología clínica:</strong> fechas de inicio de síntomas y de primera visita al hospital.</li>
            <li><strong>Desenlace:</strong> si el paciente falleció o se recuperó al momento del registro.</li>
            <li><strong>Descripción del caso:</strong> notas en texto sobre el historial del paciente.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        st.markdown("""
        <div class="card" style="color-scheme:light;">
        <p style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#64748b !important;margin-bottom:0.8rem;">Limitaciones importantes</p>
        <ul style="font-size:0.9rem;color:#374151 !important;line-height:1.9;padding-left:1.2rem;">
            <li>Cubre únicamente el inicio de la pandemia, no su impacto total.</li>
            <li>Gran parte de los casos no tenían desenlace definido al registrarse.</li>
            <li>La recopilación de datos era incompleta y puede tener sesgos.</li>
            <li>Este análisis es exploratorio y no reemplaza estudios clínicos formales.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class="card" style="color-scheme:light;">
    <p style="font-size:0.75rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#64748b !important;margin-bottom:0.8rem;">Como mantener este tablero actualizado</p>
    <ol style="font-size:0.9rem;color:#374151 !important;line-height:1.9;padding-left:1.2rem;">
        <li><strong>Revisar Kaggle</strong> periódicamente para verificar si existen versiones más completas del dataset.</li>
        <li><strong>Our World in Data</strong> (ourworldindata.org/covid-deaths) es una fuente complementaria con datos actualizados a nivel de países.</li>
